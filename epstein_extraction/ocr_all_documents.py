"""
OCR ALL documents - including large ones (>50 images).
Designed for distributed processing across multiple machines.

Run on both machines:
    python ocr_all_documents.py
...
PG_CONN = {
    'host': 'localhost',  # Change to 'BOBBYHOMEEP' for remote machine
    'database': 'epstein_documents',
    'user': 'epstein_user',
    'password': 'epstein_secure_pw_2024'
}

# For remote machine, set this to the network share path
IS_REMOTE = False  # Set to True on remote machine
...
def translate_path(local_path):
    """Convert local path to UNC path for network access (remote machine only)."""
    if IS_REMOTE and local_path and local_path.startswith(LOCAL_BASE):
...
def main():
    logger.info("=" * 70)
    logger.info("OCR ALL DOCUMENTS (including large ones)")
    logger.info("=" * 70)
    logger.info(f"Database: {PG_CONN['host']}")
    logger.info(f"Remote mode: {IS_REMOTE}")
